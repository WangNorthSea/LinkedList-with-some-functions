...
int main(void) {
    ...
    double newData;
    printf("Please enter the position of the data you want to change(start from 0):");
    scanf("%d", &pos);
    printf("Please enter the new data:");
    scanf("%lf", &newData);
    if (changeData(pHead, pos, newData)) {printf("change data succeeded!\n");}
    else {printf("change data failed!\n");}
    
    if (showData(pHead)) {printf("show changed data succeeded!\n");}
    else {printf("show changed data failed!\n");}
